...
Status encode_int_data_to_image(int data,int size,EncodeInfo *encInfo)
{
	for(int i=0; i<size; i++)
	{

		fread(encInfo->Max_image_data,32,1,encInfo->fptr_src_image);
		encode_size_to_lsb(data,encInfo->Max_image_data);
		fwrite(encInfo->Max_image_data,32,1,encInfo->fptr_stego_image);
	}
}
Status encode_secret_file_extn_size(int file_extn_size, EncodeInfo *encInfo)
{
	encode_int_data_to_image(file_extn_size,1,encInfo);
	return e_success;
}
...
Status encode_long_size_to_lsb(long data,char *image_buffer)
{
    for(int i=0;i<64;i++)
        {
          image_buffer[i]=(image_buffer[i]&0xFE)|(data>>i)&1;
...
Status encode_long_int_data_to_image(long data,int size,EncodeInfo *encInfo)
{
	for(int i=0; i<size; i++)
        {
                fread(encInfo->Max_image_long_data,64,1,encInfo->fptr_src_image);
                encode_long_size_to_lsb(data,encInfo->Max_image_long_data);
                fwrite(encInfo->Max_image_long_data,64,1,encInfo->fptr_stego_image);
        }
}
Status encode_secret_file_size(long file_size, EncodeInfo *encInfo)
{
	encode_long_int_data_to_image(file_size,1,encInfo);
	return e_success;
}
...
{
	char ch;
	while(fread(&ch,1,sizeof(ch),fptr_src)>0)
	{
...
Status check_capacity(EncodeInfo *encInfo)
{
   encInfo->image_capacity=get_image_size_for_bmp(encInfo->fptr_src_image);
    ...
    printf("%ld\n",encInfo->size_secret_file);
	    if((encInfo->image_capacity) > (16+32+32+64+encInfo->size_secret_file*8))
	    {
		    return e_success;
	    }
	    else
...
Status do_encoding(EncodeInfo *encInfo)
{
   ...
   {
	   printf("Info : Open files is sucess\n");
	   if(check_capacity(encInfo)==e_success)
	   {
		   ...
		   {
			   printf("copied bmp header unsuccessfully\n");
			   return e_failure;
		   }
	   }
	   else
	   {
		   printf("secret data does not fit in original image\n");
		   return e_failure;
	   }
   }       
   else
